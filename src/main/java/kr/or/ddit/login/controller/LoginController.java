...
* </pre>
*/
// web.xml servlet, servlet-mapping --> java annotation(주석)
@WebServlet("/login")
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		logger.debug("parameter rememberme : {}", request.getParameter("UNT_CD"));
		...
		// /login/login.jsp로 위임 
		//	 -> 서버상에 별도의 상태 변경을 가하는 요청이 아니기 때문에 dispatch 방식으로 위임
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userVO != null && encryptPassword.equals(userVO.getPass())){
			...
			//remember 파라미터가 존재할 경우 userId, rememberme cookie 설정
			if(request.getParameter("rememberme") != null) {
				cookieMaxAge = 60*60*24*30;
			}
				
			//remember 파라미터가 존재하지 않을 경우 userId, rememberme cookie 삭제
		...
		}else { // 불일치하면 -> 아이디 혹은 비밀번호를 잘못 입력 -> login 화면으로 이동
